---
title: '[Tugas] Pertemuan Minggu 13 - PHP'
...
Pada pertemuan ke-13 kelas PWEB D, diberikan tugas untuk membuat CRUD (Create Read Update Delete) sistem informasi pendaftaran sekolah.
Untuk kode program php, saya buatkan API di hosting pribadi saya yaitu [**https://ariesta.club/**](https://ariesta.club/).

Hasilnya adalah sebagai berikut : [**Tugas Pertemuan 13**](/pertemuan-13-php/index.html)

Halaman **/index.html**
![Halaman Utama](/img/php/home.png)

Halaman **/daftar.html**
![Daftar](/img/php/daftar.png)

Halaman **/pendaftar.html**
![Pendaftar](/img/php/pendaftar.png)
...
<script>
    ...
    $('#submit-btn').on('click', () => {
        ...
        let ERROR = false;
        if(data.nama.length == 0) { ERROR = true }
        if(data.kelamin.length == 0) { ERROR = true }
        if(data.agama.length == 0) { ERROR = true }
        if(data.sekolah.length == 0) { ERROR = true }
        if(data.alamat.length == 0) { ERROR = true }
